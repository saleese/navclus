...
//					((GraphNode) itemUnderMouse).highlight(); // salee: remove
					fireWidgetSelectedEvent(itemUnderMouse);
				} else if (itemUnderMouse.getItemType() == GraphItem.CONNECTION) {
					selectedItems.add(itemUnderMouse);
//					((GraphConnection) itemUnderMouse).highlight(); // salee: remove
					fireWidgetSelectedEvent(itemUnderMouse);
	...
	// salee: create
	public void changeNode(GraphNode node, IFigure newFigure) {
		// remove nodes
		IFigure figure = node.getNodeFigure();
...
//		this.getNodes().remove(node);
		...
		// change figure
		node.setData(newFigure);
		node.nodeFigure = newFigure;

		// add nodes
//		this.getNodes().add(node);
		zestRootLayer.addNode(node.getFigure());
		figure2ItemMap.put(newFigure, node);
	}
	...
	// salee: create
	private void applyNoLayoutInternal() {
		...
		try {
			Animation.markBegin();
			layoutAlgorithm.applyNoLayout(nodesToLayout, connectionsToLayout, 0, 0, d.width, d.height, false, false);
			Animation.run(ANIMATION_TIME);
			getLightweightSystem().getUpdateManager().performUpdate();
