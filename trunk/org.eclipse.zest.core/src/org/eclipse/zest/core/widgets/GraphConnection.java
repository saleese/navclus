...
public class GraphConnection extends GraphItem {
	...
	// added by salee
	private int arrowTip = ARROW_TIP_NONE;
	...
	private void doUpdateFigure(Connection connection) {
		...
		if ((connectionStyle & ZestStyles.CONNECTIONS_DIRECTED) > 0) {
			...
			// changed by salee
			if ((this.arrowTip == ARROW_TIP_START) || (this.arrowTip == ARROW_TIP_BOTH)) {
				((PolylineConnection) connection).setSourceDecoration(decoration);
			}
			if ((this.arrowTip == ARROW_TIP_END) || (this.arrowTip == ARROW_TIP_BOTH)) {
