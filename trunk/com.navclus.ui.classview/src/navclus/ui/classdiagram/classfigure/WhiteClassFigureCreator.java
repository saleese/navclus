...
public class WhiteClassFigureCreator {

	Color classColor = new Color(null, 255, 255, 255); 
	Font  classFont = new Font(null, "Arial", 12, SWT.BOLD);
	
	public WhiteClassFigureCreator() {
		super();
	}
		...
		if (itype != null) {
			try {			
				if (itype.isInterface()) {
					return  new Label(itype.getElementName(), Activator.getDefault().getImage("int_obj"));					
				}
				else if (itype.isEnum()) {
					return  new Label(itype.getElementName(), Activator.getDefault().getImage("enum_obj"));
				}
				else {
					return  new Label(itype.getElementName(), Activator.getDefault().getImage("class_obj"));
				}
		...
		try {
			for (IMethod imethod: Methods) {
				...
				if (Flags.isPublic(iFlag)) {
					method = new Label(methodname, Activator.getDefault().getImage("methpub_obj"));
				}
				else if (Flags.isPrivate(iFlag)) {
					method = new Label(methodname, Activator.getDefault().getImage("methpri_obj"));
				}
				else if (Flags.isProtected(iFlag)) {
					method = new Label(methodname, Activator.getDefault().getImage("methpro_obj"));
				}
				else {
					method = new Label(methodname, Activator.getDefault().getImage("methdef_obj"));
				}					
	...
	public void addFields(ClassFigure classFigure, Set<IField> Fields) {
		...
		try {			
			for (IField ifield: Fields) {
				...
				if (Flags.isPublic(iFlag)) {
					attribute = new Label(fieldname, Activator.getDefault().getImage("field_public_obj"));				
				}
				else if (Flags.isPrivate(iFlag)) {
					attribute = new Label(fieldname, Activator.getDefault().getImage("field_private_obj"));
				}
				else if (Flags.isProtected(iFlag)) {
					attribute = new Label(fieldname, Activator.getDefault().getImage("field_protected_obj"));
				}
				else {
					attribute = new Label(fieldname, Activator.getDefault().getImage("field_default_obj"));
				}		
