...
package navclus.ui.classdiagram.listeners;
...
public class GraphMouseListener implements org.eclipse.draw2d.MouseListener {
	...
	public void mouseDoubleClicked(MouseEvent me) {
		...
		if (ClassView.getDefault() == null) return; 
		if (ClassView.getDefault().getG() == null) return;
		
		RootNode root = ClassView.getDefault().getRootNode();	
		
		Point mousePoint = new Point(me.x, me.y);
		ClassView.getDefault().getG().getRootLayer().translateToRelative(mousePoint);
		ClassView.getDefault().getG().getRootLayer().translateFromParent(mousePoint);
		
		List selectedItems = ClassView.getDefault().getG().getSelection();
